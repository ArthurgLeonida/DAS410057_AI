(define (domain cafeteria-hot)
(:requirements :strips :typing :durative-actions :duration-inequalities :fluents)
...
(:durative-action prepare-cold
  ...
  :effect (and (at start (not (barista-free))) (at start (not (pending ?o)))
               (at end (barista-free)) (at end (ready ?o)) (at end (valid ?o))))
...
(:durative-action load-hand
  ...
  :condition (and (over all (at ?w bar)) (at start (hand-free ?w))
                  (over all (no-tray ?w))
                  (at start (ready ?o)) (at start (valid ?o)))
